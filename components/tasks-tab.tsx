"use client"

import { useState, useEffect } from "react"
import { Calendar, Tag, ChevronDown, ChevronUp, Check, User, Trophy } from "lucide-react"
// No direct supabase client needed here if all fetching is through context
// import { createClientSupabaseClient } from "@/lib/supabase"
import { useUser } from "@/components/UserContext"
import { Button } from "@/components/ui/button"
import { Card, CardContent } from "@/components/ui/card"
import { Avatar, AvatarFallback } from "@/components/ui/avatar"
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter } from "@/components/ui/dialog"
import Link from "next/link"
import TaskUpdater from "@/components/TaskUpdater"
import { useTaskVerification } from '@/hooks/useTaskVerification'
import type { UserTask } from '@/types/supabase'; // Import UserTask type if not already global

// Define level thresholds (adjust if needed)
const levelThresholds = [
  { level: 1, core: 2 },
  { level: 2, core: 4 },
  { level: 3, core: 8 },
  { level: 4, core: 16 },
  { level: 5, core: 32 },
  { level: 6, core: 64 },
  { level: 7, core: 125 },
  { level: 8, core: 250 },
  { level: 9, core: 500 },
  { level: 10, core: 1000 },
  { level: 11, core: 2000 },
  // Add more levels as needed following the doubling pattern (or adjust)
];

// Function to calculate level based on core
const calculateLevel = (core: number): number => {
  let calculatedLevel = 0;
  for (let i = levelThresholds.length - 1; i >= 0; i--) {
    if (core >= levelThresholds[i].core) {
      calculatedLevel = levelThresholds[i].level;
      break; // Found the highest applicable level
    }
  }
  return calculatedLevel; // Return 0 if core is below the first threshold
};

// Initialize Supabase client ONLY if still needed for direct updates (like level)
// If level updates could also be managed via context/API calls, this could be removed too.
import { createClientSupabaseClient } from "@/lib/supabase";
const supabase = createClientSupabaseClient();

// Remove local Task type if UserTask from context covers it
/*
type Task = {
  number: number
  title: string
  reward: number
  icon_url: string
  description: string
  completion_condition: string
  due_date: string | null
  notes: string
  status: string
  assigned_at: string
  current_step_index: number
  progress_details: string
  steps_total?: number
}
*/

export default function TasksTab() {
  // Get tasks, refresh function, loading and error states from context
  const {
    dbUser,
    isLoading: isUserLoading,
    refreshUserData,
    tasks: contextTasks, // Rename to avoid conflict if needed, or use directly
    refreshTasks,
    error: contextError,
    isLoading: isContextLoading // Use context loading state
  } = useUser();

  const [activeTab, setActiveTab] = useState("new")
  const [expandedTaskId, setExpandedTaskId] = useState<number | null>(null)
  // Remove local task state
  // const [tasks, setTasks] = useState<Task[]>([])
  // Use context loading state, potentially combine with initial load check if needed
  // const [loading, setLoading] = useState(true)
  // Use context error state
  // const [error, setError] = useState<string | null>(null)
  const [statusMessage, setStatusMessage] = useState<{ type: 'success' | 'error', text: string } | null>(null)
  const [completionModal, setCompletionModal] = useState<{
    isOpen: boolean;
    taskNumber: number; // Use task.number instead of task.id if 'number' is the identifier
    reward: number;
    oldCore?: number;
    newCore?: number;
  } | null>(null)
  const [levelUpModal, setLevelUpModal] = useState<{
    isOpen: boolean;
    newLevel: number | null;
    oldLevel: number | null; // Optional: To show the transition
  } | null>(null);
  const [isUpdatingLevel, setIsUpdatingLevel] = useState(false); // To prevent multiple updates

  // Ensure hook uses refreshTasks from context
  const { verifying, handleTaskVerification } = useTaskVerification({
    dbUser,
    refreshUserData, // Keep for user profile updates
    // refreshTasks, // Pass refreshTasks to the hook - REMOVED, called on modal close
    setStatusMessage,
    onTaskComplete: (taskNumber: number, reward: number, oldCore: number, newCore: number) => {
      setCompletionModal({
        isOpen: true,
        taskNumber,
        reward,
        oldCore,
        newCore
      });
      // No need to call fetchTasks here, refreshTasks should be called by the hook or after modal close
    }
  })

  // Remove useEffect that calls local fetchTasks
  /*
  useEffect(() => {
    if (dbUser?.id) {
      fetchTasks()
    } else if (!isUserLoading) {
      setLoading(false)
    }
  }, [dbUser?.id, isUserLoading])
  */

  // Effect to check for level up (keep as is, uses dbUser)
  useEffect(() => {
    if (dbUser && !isUserLoading && !isUpdatingLevel) {
      const currentAiCoreBalance = dbUser.aicore_balance || 0;
      const currentLevel = dbUser.level || 0; // Assuming dbUser has a 'level' field
      const calculatedLevel = calculateLevel(currentAiCoreBalance);

      console.log(`Checking level: AICoreBalance=${currentAiCoreBalance}, CurrentLevel=${currentLevel}, CalculatedLevel=${calculatedLevel}`); // Update log

      if (calculatedLevel > currentLevel) {
        console.log(`Level up detected! ${currentLevel} -> ${calculatedLevel}`); // Debug log
        setIsUpdatingLevel(true); // Lock to prevent race conditions

        // Update level in Supabase - This direct update might be okay,
        // or could be moved to an API call triggered by context if preferred.
        const updateUserLevel = async () => {
          if (!dbUser?.id) {
             setIsUpdatingLevel(false);
             return;
          }
          try {
            // Ensure supabase client is available
             if (!supabase) {
                console.error("Supabase client not available for level update");
                setStatusMessage({ type: 'error', text: 'Cannot update level: connection error.' });
                setIsUpdatingLevel(false);
                return;
             }
            const { error } = await supabase
              .from('users') // Make sure 'users' table name is correct
              .update({ level: calculatedLevel }) // Make sure 'level' column name is correct
              .eq('id', dbUser.id);

            if (error) {
              console.error("Error updating user level:", error);
              setStatusMessage({ type: 'error', text: `Failed to update level: ${error.message}` });
              setIsUpdatingLevel(false); // Unlock on error
            } else {
              console.log(`User level updated successfully to ${calculatedLevel}`); // Debug log
              // Show level up modal AFTER db update is successful
              setLevelUpModal({
                isOpen: true,
                newLevel: calculatedLevel,
                oldLevel: currentLevel
              });
              // Refresh user data to get the updated level from DB *after* modal is closed
              // No need to set isUpdatingLevel back immediately, let the modal close handle it
            }
          } catch (err) {
            console.error("Unexpected error updating level:", err);
            setStatusMessage({ type: 'error', text: 'An unexpected error occurred while updating level.' });
            setIsUpdatingLevel(false); // Unlock on unexpected error
          }
        };

        updateUserLevel();
      }
    }
    // Ensure all dependencies are correct, including supabase if used directly
  }, [dbUser?.aicore_balance, dbUser?.level, isUserLoading, isUpdatingLevel, dbUser?.id, refreshUserData, setStatusMessage, supabase]);

  // Remove local fetchTasks function
  /*
  const fetchTasks = async () => {
    if (!dbUser?.id) return;

    try {
      setLoading(true)
      setError(null)

      const { data, error } = await supabase
        .from("user_tasks")
        .select(\`
          *,
          task:tasks(*)
        \`)
        .eq("user_id", dbUser.id)
        .order("assigned_at", { ascending: false })

      if (error) {
        console.error("Error fetching tasks:", error)
        setError("Failed to fetch tasks. Please try again later.")
      } else {
        // Transform the data to match the Task interface
        const transformedTasks = data.map(userTask => ({
          ...userTask.task,
          status: userTask.status,
          assigned_at: userTask.assigned_at,
          current_step_index: userTask.current_step_index,
          progress_details: userTask.progress_details,
          steps_total: userTask.task.steps_total
        }))
        setTasks(transformedTasks)
      }
    } catch (err) {
      console.error("Unexpected error:", err)
      setError("An unexpected error occurred")
    } finally {
      setLoading(false)
    }
  }
  */

  const toggleTaskExpansion = (taskNumber: number) => {
    // Use task.number if that's the identifier from UserTask type
    setExpandedTaskId(prevId => (prevId === taskNumber ? null : taskNumber));
  };

  const getTaskIdentifier = (task: UserTask): number => {
     // Adjust this based on the actual structure of UserTask and the nested task object
     // Assuming the unique identifier is `task.task.number`
     // Use type assertion as workaround for incorrect UserTask type
     // Use optional chaining and nullish coalescing for safety
     return (task as any).task?.number ?? task.id ?? -1; // Fallback to userTask.id if task.number is missing
  }

  // Use tasks from context
  const tasks = contextTasks || [];

  const filteredTasks = () => {
    switch (activeTab) {
      case "new":
        return tasks.filter((userTask) => (userTask.status as string) === 'assigned' || (userTask.status as string) === 'in_progress')
      case "completed":
        return tasks.filter((userTask) => (userTask.status as string) === 'completed')
      default:
        return tasks.filter((userTask) => (userTask.status as string) === 'assigned' || (userTask.status as string) === 'in_progress')
    }
  }

  // Use context loading state
  if (isContextLoading && tasks.length === 0) { // Show loading only on initial load perhaps
    return <div className="flex items-center justify-center h-full">Loading user data and tasks...</div>
  }

  // Handle case where user is not logged in (as before)
  if (!dbUser?.id && !isContextLoading) {
    return (
      <div className="p-4">
        <Card>
          <CardContent className="pt-6">
            <div className="text-center flex flex-col items-center gap-4">
              <h3 className="text-lg font-medium">Access to Tasks</h3>
              <p className="text-gray-500 mb-4">To access your tasks, please log in to the system</p>
              <Avatar className="h-20 w-20 mx-auto mb-2">
                <AvatarFallback>
                  <User className="h-10 w-10 text-gray-400" />
                </AvatarFallback>
              </Avatar>

              <Button
                className="bg-purple-600 hover:bg-purple-700 text-white flex items-center gap-2 px-6"
                onClick={() => {
                  const socialButton = document.querySelector('button[aria-label="Social"]');
                  if (socialButton instanceof HTMLElement) {
                    socialButton.click();
                  }
                }}
              >
                <User className="h-5 w-5" />
                Go to Profile
              </Button>

              <p className="text-xs text-gray-400 mt-4">
                After logging in, you will get access to all application features
              </p>
            </div>
          </CardContent>
        </Card>
      </div>
    )
  }

  // Use context error state
  if (contextError) {
    return <div className="flex items-center justify-center h-full text-red-500">Error: {contextError}</div>
  }

  return (
    <div className="flex flex-col h-full bg-gray-50">
      {/* Status Message */}
      {statusMessage && (
        <div className={`p-3 text-sm ${
          statusMessage.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
        } transition-opacity duration-500`}>
          {statusMessage.text}
        </div>
      )}

      {/* Completion Modal */}
      <Dialog open={completionModal?.isOpen} onOpenChange={(open) => {
        if (!open) {
          setCompletionModal(null);
          refreshTasks(); // Refresh tasks from context when closing the modal
        }
      }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-center flex items-center justify-center gap-2">
              <Trophy className="h-6 w-6 text-yellow-500" />
              Task Completed!
            </DialogTitle>
          </DialogHeader>
          <div className="py-6">
            <div className="text-center space-y-4">
              <p className="text-lg font-medium">Congratulations!</p>
              <p className="text-gray-600">
                {/* Use task.number from the modal state */}
                You've successfully completed Task {completionModal?.taskNumber}
              </p>
              <div className="bg-purple-50 p-4 rounded-lg">
                <p className="text-sm text-purple-700 mb-2">Rewards:</p>
                <div className="flex justify-center items-center gap-2 text-2xl font-bold text-purple-700">
                  {/* Use reward from the modal state */}
                  +${completionModal?.reward}
                </div>
              </div>
              <div className="bg-blue-50 p-4 rounded-lg">
                <p className="text-sm text-blue-700 mb-2">Core Progress:</p>
                <div className="flex justify-center items-center gap-2">
                   {/* Use core values from the modal state */}
                  <span className="text-xl text-blue-700">${completionModal?.oldCore ?? 0}</span>
                  <ChevronUp className="h-6 w-6 text-green-500" />
                  <span className="text-2xl font-bold text-blue-700">${completionModal?.newCore ?? 0}</span>
                </div>
              </div>
            </div>
          </div>
          <DialogFooter>
            <Button
              className="w-full"
              onClick={() => {
                setCompletionModal(null);
                refreshTasks(); // Refresh tasks from context when clicking Continue
              }}
            >
              Continue
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* Level Up Modal (logic remains similar, ensure refreshUserData is called) */}
      <Dialog open={levelUpModal?.isOpen} onOpenChange={(open) => {
        if (!open) {
          setLevelUpModal(null);
          setIsUpdatingLevel(false); // Unlock when modal closes
          refreshUserData(); // Refresh user data to ensure UI consistency
        }
      }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-center flex items-center justify-center gap-2">
              <Trophy className="h-6 w-6 text-yellow-500" />
              Level Up!
            </DialogTitle>
          </DialogHeader>
          <div className="py-6">
            <div className="text-center space-y-4">
              <p className="text-lg font-medium">Congratulations!</p>
              <p className="text-gray-600">
                You've reached Level <span className="font-bold text-purple-600">{levelUpModal?.newLevel}</span>!
              </p>
              <div className="bg-purple-50 p-4 rounded-lg">
                 <p className="text-sm text-purple-700 mb-2">Keep growing your Core!</p>
                 {/* You could add the next level's requirement here */}
                 {levelUpModal?.newLevel && levelUpModal.newLevel < levelThresholds.length && (
                    <p className="text-xs text-purple-600">
                        Next level at ${levelThresholds[levelUpModal.newLevel].core} Core.
                    </p>
                 )}
              </div>
            </div>
          </div>
          <DialogFooter>
            <Button
              className="w-full"
              onClick={() => {
                setLevelUpModal(null);
                setIsUpdatingLevel(false); // Unlock when modal closes
                refreshUserData(); // Refresh user data
              }}
            >
              Awesome!
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* Tabs and TaskUpdater - Use refreshTasks from context */}
      <div className="flex items-center justify-between p-2 bg-white border-b">
        <div className="flex space-x-2">
          <button
            className={`px-4 py-2 rounded-full text-sm font-medium ${
              activeTab === "new" ? "bg-purple-500 text-white" : "bg-white text-gray-700"
            }`}
            onClick={() => setActiveTab("new")}
          >
            New
          </button>
          <button
            className={`px-4 py-2 rounded-full text-sm font-medium ${
              activeTab === "completed" ? "bg-purple-500 text-white" : "bg-white text-gray-700"
            }`}
            onClick={() => setActiveTab("completed")}
          >
            Completed
          </button>
        </div>
        {/* Pass refreshTasks from context to the updater */}
        <TaskUpdater onUpdate={refreshTasks} />
      </div>

      {/* Task list - Iterate over tasks from context */}
      <div className="flex-1 overflow-y-auto">
        {/* Make sure the structure matches UserTask[] */}
        {filteredTasks().map((userTask) => {
          // Use type assertion as workaround for incorrect UserTask type
          const task = (userTask as any).task;
          const taskNumber = getTaskIdentifier(userTask); // Get unique ID

           if (!task) return null; // Skip if nested task data is missing

           return (
            <div key={taskNumber} className="border-b"> {/* Use unique ID as key */}
              {/* Task header - Modified to include button */}
              <div className="flex items-center p-4">
                {/* Icon */}
                <img
                  // Use properties from nested task object
                  src={task.icon_url || "/placeholder.svg"}
                  alt={task.title}
                  className="flex-shrink-0 w-8 h-8 rounded-full object-cover mr-3"
                />

                {/* Title and Meta (Takes remaining space) */}
                 {/* Use task.number for toggle expansion */}
                <div className="flex-1 min-w-0 mr-4" onClick={() => toggleTaskExpansion(taskNumber)} style={{ cursor: 'pointer' }}>
                  <h3 className="text-base text-gray-800 truncate">
                    {/* Use properties from nested task object */}
                    {task.title}
                  </h3>
                  <div className="flex flex-wrap items-center mt-1 text-xs text-gray-500">
                    <div className="flex items-center mr-3">
                      <Calendar className="h-3 w-3 mr-1" />
                      {/* Use properties from userTask object */}
                      {userTask.assigned_at ? new Date(userTask.assigned_at).toLocaleDateString() : "Not started"}
                    </div>
                    <div className="flex items-center mr-3">
                      <Tag className="h-3 w-3 mr-1" />
                      {/* Use properties from nested task object */}
                      {task.reward}$
                    </div>
                    <div className={`flex items-center ${
                      // Use properties from userTask object
                      userTask.status === 'completed' ? 'text-green-500' :
                      userTask.status === 'in_progress' ? 'text-blue-500' :
                      'text-gray-500'
                    }`}>
                      {userTask.status}
                    </div>
                  </div>
                </div>

                {/* Actions (Button and Chevron) */}
                <div className="flex items-center space-x-2">
                   {/* Use userTask status and pass taskNumber to handler */}
                  {userTask.status !== 'completed' && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={(e) => {
                        e.stopPropagation();
                        // Pass the correct identifier to the verification handler
                        handleTaskVerification(taskNumber);
                      }}
                      disabled={verifying}
                    >
                      {verifying ? (
                        <span className="animate-spin rounded-full h-4 w-4 border-b-2 border-gray-800 mr-2"></span>
                      ) : (
                        <Check className="h-4 w-4 mr-2" />
                      )}
                      Check
                    </Button>
                  )}
                   {/* Use taskNumber for toggle expansion */}
                  <div onClick={() => toggleTaskExpansion(taskNumber)} style={{ cursor: 'pointer' }}>
                    {expandedTaskId === taskNumber ? (
                      <ChevronUp className="h-5 w-5 text-gray-400" />
                    ) : (
                      <ChevronDown className="h-5 w-5 text-gray-400" />
                    )}
                  </div>
                </div>
              </div>

              {/* Expanded task details */}
               {/* Use taskNumber for checking expansion */}
              {expandedTaskId === taskNumber && (
                <div className="px-4 pb-4 pt-0">
                  <div className="ml-11">
                    <div className="mb-3">
                       {/* Use properties from nested task object */}
                      <p className="text-sm text-gray-700">{task.description}</p>
                    </div>

                    {/* Use properties from userTask object */}
                    {userTask.current_step_index !== null && typeof userTask.current_step_index === 'number' && task.steps_total && (
                      <div className="bg-gray-50 p-3 rounded-lg mb-3">
                        <h4 className="text-xs font-medium text-gray-700 mb-2">Progress:</h4>
                        <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
                          <div
                            className="bg-purple-600 h-2 rounded-full"
                             // Use userTask properties and task steps_total
                            style={{ width: `${((userTask.current_step_index ?? 0) / (task.steps_total ?? 1)) * 100}%` }}
                          ></div>
                        </div>
                         {/* Use userTask properties and task steps_total */}
                        <p className="text-xs text-gray-600">Step {(userTask.current_step_index ?? 0) + 1} of {task.steps_total ?? '?'}</p>
                      </div>
                    )}

                    {/* Use properties from nested task object */}
                    {task.notes && (
                      <div className="bg-gray-50 p-3 rounded-lg">
                        <h4 className="text-xs font-medium text-gray-700 mb-1">Notes:</h4>
                        <p className="text-xs text-gray-600">{task.notes}</p>
                      </div>
                    )}
                  </div>
                </div>
              )}
            </div>
           );
        })}
      </div>
    </div>
  )
}

