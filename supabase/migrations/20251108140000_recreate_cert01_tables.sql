-- Migration: Recreate certificate tables with consolidated user interactions
-- This migration handles the transition from fragmented to consolidated table structure

-- Drop existing tables if they exist (to ensure clean slate)
DROP TABLE IF EXISTS public.user_certificate_interactions CASCADE;
DROP TABLE IF EXISTS public.certificate_cards CASCADE;

-- Recreate certificate_cards table
...
-- Recreate user_certificate_interactions table (consolidated - one row per user-card)
...
-- user_certificate_interactions policies (consolidated)
-- Temporarily allow all operations for debugging
CREATE POLICY "Allow all operations for debugging" ON public.user_certificate_interactions
    FOR ALL USING (true);
