...
CREATE INDEX IF NOT EXISTS core_operations_user_id_idx ON core_operations(user_id);
CREATE INDEX IF NOT EXISTS core_operations_created_at_idx ON core_operations(created_at DESC);

-- Add RLS policies
ALTER TABLE core_operations ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own operations"
  ON core_operations FOR SELECT
